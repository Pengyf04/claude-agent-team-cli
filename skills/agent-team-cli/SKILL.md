...
3. 确认你自己的会话名：默认约定用户以 `claude --name main` 启动了你；若用户用了其他名字（或不确定），让用户告知——该名字要传给启动脚本，角色首次 READY 按它寻址。**主控也可以是 Claude Code 桌面版会话**（不能 `--name`，注册名由系统派生）：此时用 Bash 执行 `cat ~/.claude/sessions/$PPID.json` 读取 `name` 字段作为主控名传给脚本（桌面版的显示名可能与注册名不同——角色识别主控靠**团队令牌**而非名字，名字只用于角色回报寻址）。
4. **团队后缀默认 = 本次任务 slug**（自动，无需用户配置）：角色会话名为 `<角色>-<slug>`，多项目并行、旧团队残留均不撞名，窗口标题也一眼可辨。主控自身命名建议：单项目 `main` 即可；多项目并行时**必须**让主控名互不相同（建议 `main-<slug>`）：角色首次 READY 握手按主控名寻址，机器上不能有两个同名主控；握手之后角色仍按主控名回报（绝不发地址）。
...
- **桌面版主控注意**：Claude Code 桌面版会话可作主控（本框架已实测），但：① 不认项目级 `settings.local.json` 的 `crossSessionInbound`，能否收到 bypass 角色的消息取决于主控自身权限模式（bypass 可收；auto/default 会被扣且**不显示批准框**）——桌面版主控请让用户把会话权限模式设为 bypassPermissions，或在用户级 `~/.claude/settings.json` 写 `crossSessionInbound: accept`；② `/status` 不可用；③ 显示名≠注册名（令牌机制已不依赖名字）。
