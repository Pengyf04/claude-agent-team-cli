...
3. **确认你自己的权限模式**：若你不是 bypassPermissions，角色（bypass）发给你的消息会被系统扣住、团队会卡死。此时请让用户以 `claude --name <你的名字> --permission-mode bypassPermissions` 重启主控后重新调用本 skill。（项目级 `crossSessionInbound: accept` 在 auto 模式下实测未生效，不要依赖它。）
...
3. 若用户选择关闭团队且本项目近期不再跑团队：问是否同时移除 `.claude/settings.local.json` 里的 `crossSessionInbound` 键（该键使本项目会话无门禁收取本机任意会话消息，不用就收回）。用户同意后**必须执行 `bash ~/.claude/skills/agent-team-cli/scripts/ensure-inbound.sh "<项目绝对路径>" --remove`**，**不要自己编辑该 JSON**——脚本只摘掉这一个键并保留文件其余内容；只有当文件里再无其他键时才会整个删除。手工编辑容易连带抹掉用户自己的项目配置。
