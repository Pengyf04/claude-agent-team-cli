...
## [重入检查] 每次被调用时最先做
若当前项目已存在 `runs/*/state.md`（首行含 `agent-team-cli/SKILL.md`）且阶段不是 `[P5] 完成`、任务与本次描述一致（或用户说"继续"）→ **不要重做 P0/P1**，按下列顺序恢复：

1. **先核对自己的注册名**：Bash 读 `~/.claude/sessions/$PPID.json` 的 `name`，与 state.md 首行记录的「main会话名」比对。
   - **不一致**（桌面版主控进程重建后必然发生，派生名每次都变）→ **停下**，告诉用户：角色仍按旧名字 `<记录名>` 寻址，请用斜杠命令 `/rename <记录名>` 改回（选 `Custom command` 那个，不是弹窗那个），改完再让你继续。**不要带着错名字往下走**——角色的回报会静默丢失。
   - 一致 → 继续下一步。
2. 读 state.md 恢复到对应阶段；`ListAgents` 确认本团队四个角色是否在线。看不到 → 让用户确认窗口是否已关，再走 P2。
3. **补 ping 正在等待的角色**：中断期间角色可能已经发过回报但你当时不可达，而角色协议规定「回报只发一次、不重发」——所以你必须主动要一次。按 state.md 的「正在等待」字段，向对应角色发一条**带令牌**的消息：「主控已恢复（会话名 `<当前名>`）。若你已完成本轮工作，请把结论重发一次；若尚未完成，忽略本条继续。」四个角色都 READY 未收齐时，则向全部四个发「请重发 READY」。
4. 收到回报后按状态机继续；**人工卡点仍需用户确认，不得因为是"恢复"就跳过**。

## [P0] 目标对齐（人工）
1. 分析任务描述；有疑问**逐个**与用户讨论对齐（AskUserQuestion 或直接问），直到目标、范围、约束清晰。
2. **必须拿到可判定的验收标准**；用户没给就先问，问不出就和用户一起拟一版并确认。
3. **确认主控能收到角色消息**（角色跑 bypassPermissions；主控收不到它们的消息 = 团队卡死，且桌面版无批准框、静默卡住）。先用 Bash 读 `~/.claude/sessions/$PPID.json`（或 `cat`）确认自己的 `entrypoint`：
   - **终端主控**（`entrypoint=cli`）：确认启动时带了 `--settings '{"crossSessionInbound":"accept"}'`。没带就让用户带同样 `--name` 加该参数重启后重新调用本 skill。
   - **桌面版主控**（`entrypoint=claude-desktop`）：桌面版**不能用 `--settings`**，改为确认**用户级** `~/.claude/settings.json` 含 `"crossSessionInbound": "accept"`（`bash <skill目录>/scripts/doctor.sh` 会检查）。缺失就让用户加上并**重启桌面客户端**（该配置启动时读取）。
   - 两者都**不要**建议改用项目级 `settings.local.json`——该键项目级写 `accept` 是空操作（见 design-decisions #6）。
4. 确认你自己的会话名（角色首次 READY 按它寻址）：
   - **终端主控**：名字来自 `--name`，天然稳定。**不能用保留字 `main`**（见下）。
   - **桌面版主控**：注册名默认是派生值（形如 `test1-4f`，且**每次会话进程重建都会变**）。让用户用**斜杠命令** `/rename atc-main` 固定它——⚠️ 客户端命令菜单里有两个 rename：选带 `Custom command` 标注的那个（走会话进程、改注册名）；另一个 `Rename this session` 是弹窗，只改对话标题、**不改注册名**（是陷阱，用错了角色照样找不到你）。你用 Bash 读 `~/.claude/sessions/$PPID.json` 的 `name` 字段确认改成功了再开团队，并把该名字传给启动脚本。
   - **主控名绝不能是 `main`**：它是 SendMessage 的保留收件人，角色回报会被拦截且无绕过，团队必然卡死在握手；`launch-team.sh` 会在开窗前拒绝该名字。多项目并行时主控名还须互不相同（建议 `atc-main-<slug>`）。
5. **团队后缀默认 = 本次任务 slug**（自动，无需用户配置）：角色会话名为 `<角色>-<slug>`，多项目并行、旧团队残留均不撞名，窗口标题也一眼可辨。主控自身命名：**绝不能用 `main`**（保留字，见上）；单项目用 `atc-main` 即可，多项目并行时**必须**让主控名互不相同（建议 `atc-main-<slug>`）：角色首次 READY 握手按主控名寻址，机器上不能有两个同名主控；握手之后角色仍按主控名回报（绝不发地址）。
...
3. 建 `runs/<slug>/`，写 `task.md`（原始任务 + 验收标准），按 schema 初始化 `state.md`。
...
- READY 被「hold/待批准」拦截或**毫无动静**：主控没有生效的 inbound accept。终端主控 → 带原 `--name` 加 `--settings '{"crossSessionInbound":"accept"}'` 重启；桌面版主控 → 检查用户级 `~/.claude/settings.json` 并重启客户端。⚠️ 桌面版**不显示批准框**，被扣消息静默过期，「没反应」和「被扣了」看起来一样——先用 doctor 查配置，别干等。
...
- **桌面版主控专项**：① inbound 只能靠**用户级** `~/.claude/settings.json` 的 `crossSessionInbound: accept`（不能 `--settings`，项目级是空操作），改完需重启客户端；② 注册名是派生的且**每次会话进程重建都会变**（客户端更新/重启/崩溃都会触发），用斜杠命令 `/rename` 固定，注意别用同名的弹窗入口；③ **不显示**跨会话消息批准框，被扣即静默；④ 没有 `/status`。
