...
3. **确认主控能收到角色消息**：角色会话跑在 bypassPermissions；若你自己**不是** bypass 类（`acceptEdits`/`auto`/`default`/`dontAsk`/`manual` 都属 prompting 类），且启动时**没带** `--settings '{"crossSessionInbound":"accept"}'`，角色的 READY 会被系统扣住、团队卡死。此时请用户以 `claude --name <你的名字> --settings '{"crossSessionInbound":"accept"}'` 重启主控后重新调用本 skill。（**不要**建议改用项目级 `settings.local.json`——该键的项目级来源只在收紧时才被采纳，写 `accept` 是空操作。）
4. 确认你自己的会话名：默认约定用户以 `claude --name atc-main` 启动了你（**主控名不能是 `main`**——它是 SendMessage 的保留收件人，角色按名字回报会被拦截且无绕过，团队必然卡死在 P3 握手；`launch-team.sh` 会在开窗前拒绝该名字）；若用户用了其他名字（或不确定），让用户告知——该名字要传给启动脚本，角色首次 READY 按它寻址。**主控也可以是 Claude Code 桌面版会话**（不能 `--name`，注册名由系统派生）：此时用 Bash 执行 `cat ~/.claude/sessions/$PPID.json` 读取 `name` 字段作为主控名传给脚本（桌面版的显示名可能与注册名不同——角色识别主控靠**团队令牌**而非名字，名字只用于角色回报寻址）。
...
- **主控收不到角色消息时**（不限桌面版）：决定因素是主控自身权限模式与显式 inbound 策略。最可靠的做法是启动主控时带 `--settings '{"crossSessionInbound":"accept"}'`（显式值优先级最高，与权限模式无关）；或让主控也跑 bypassPermissions（靠模式对等）。桌面版另有：不显示批准框（被扣消息静默过期）、无 `/status`。
