...
if osascript -e 'tell application "Terminal" to get name' >/dev/null 2>&1; then ok "可通过 AppleScript 控制 Terminal.app（自动化授权已通过）"; else warn "无法控制 Terminal.app：首次运行会弹「自动化」授权，请允许；若已拒绝，到 系统设置→隐私与安全性→自动化 开启"; fi
...
echo "  planner/plan-reviewer/verifier 模型: ${ATC_MODEL_DEFAULT:-claude-fable-5}（ATC_MODEL_DEFAULT / ATC_MODEL_<ROLE> 可覆盖）"
echo "  executor 模型: ${ATC_MODEL_EXECUTOR:-claude-opus-5}   effort: ${ATC_EFFORT_DEFAULT:-xhigh}/${ATC_EFFORT_EXECUTOR:-high}   权限: ${ATC_PERMISSION_MODE:-bypassPermissions}"
warn "请确认你的账号能使用上述模型；无 Fable 权限时可 export ATC_MODEL_DEFAULT=opus（或 sonnet）"
...
if [ -f "$PROJ/.claude/settings.local.json" ] && grep -q '"crossSessionInbound"[[:space:]]*:[[:space:]]*"accept"' "$PROJ/.claude/settings.local.json"; then ok "项目已配置 crossSessionInbound=accept"; else warn "项目尚未配置 crossSessionInbound=accept（skill 会在 P1 自动写入；写入后需重启 main 会话一次）"; fi
