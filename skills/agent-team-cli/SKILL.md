...
3. 确认你自己的会话名：默认约定用户以 `claude --name main` 启动了你；若用户用了其他名字（或不确定），让用户告知——该名字要传给启动脚本，角色首次 READY 按它寻址（桌面版会话的显示名可能与注册名不同，角色端已用 TOFU 兜底）。
4. **团队后缀默认 = 本次任务 slug**（自动，无需用户配置）：角色会话名为 `<角色>-<slug>`，多项目并行、旧团队残留均不撞名，窗口标题也一眼可辨。主控自身命名建议：单项目 `main` 即可；多项目并行时**必须**让主控名互不相同（建议 `main-<slug>`）：角色首次 READY 握手按主控名寻址，机器上不能有两个同名主控；握手之后角色都按地址回报，不再依赖名字。
