...
3. 确认你自己的会话名：默认约定用户以 `claude --name main` 启动了你；若用户用了其他名字（或不确定），让用户告知——该名字要传给启动脚本，角色们只执行来自它的指令。可用 `cat ~/.claude/sessions/$PPID.json` 之类方式或 `/status` 查看自己的注册名（桌面版会话的显示名可能与注册名不同，角色端已用 TOFU 兜底）。
4. **团队后缀默认 = 本次任务 slug**（自动，无需用户配置）：角色会话名为 `<角色>-<slug>`，多项目并行、旧团队残留均不撞名，窗口标题也一眼可辨。主控自身命名建议：单项目 `main` 即可；多项目并行时建议 `main-<slug>`（角色首次 READY 握手按主控名寻址，机器上不能有两个同名主控；握手之后角色都按地址回报，不再依赖名字）。

## [P1] 环境准备
1. 确认当前目录即目标项目目录。
2. **git 兜底检查**：不是 git 仓库 → `git init` 并做一次初始提交（告知用户：子会话运行在 bypassPermissions 模式，git 是回滚兜底）。已是仓库但有未提交改动 → 建议用户先提交一次。
3. **消息通路配置**：确保项目 **`.claude/settings.local.json`**（本地文件，不进 git——不要写进共享的 settings.json）含 `"crossSessionInbound": "accept"`（merge 写入，不覆盖其他键）。若该键为本次新写入 → 告知用户：main 启动时未加载它，稍后 READY 若被「hold/待批准」拦截，需**带同样的 --name 重启 main** 后重新调用本 skill（重跑会自动跳过已完成步骤，见 P2 幂等说明）。
...
1. 运行（用 Bash 工具）：
   ...
   脚本行为：开 4 个 Terminal 窗口（主屏均分：main 左 1/3，四角色右 2/3 田字格）、以预设 name/model/effort/bypassPermissions/inbound-accept/角色 system prompt 启动会话；窗口 ID **边开边记**入项目 `.claude/agent-team-cli/windows.txt`。
   - ⚠️ 此命令要控制 Terminal.app（AppleScript），沙箱会拦——需以非沙箱方式执行；首次运行 macOS 会弹「自动化」授权，请用户点允许。
...
### 🔴 人工卡点 A：plan 定稿确认
向用户播报定稿 `plan.md` 摘要（目标、关键步骤、成功标准），**等待用户确认或要求修改**。要改 → 带意见回 [1]（规划环轮次重新计数）；确认 → 进 [2]。
...
### 🔴 人工卡点 B：最终验收确认
向用户汇总：交付物位置 + 验证报告结论 + 终验结论 + **⚠️ 风险与注意事项**（没有写"无"）。**等待用户确认**。不满 → 带反馈回 [2]；确认 → 进 [P5]。

## [P5] 收尾
1. 输出总结（做了什么、交付物在哪、验证情况、关键决策、⚠️ 风险与注意事项）。
2. 问用户是否关闭团队窗口：是 → `bash ~/.claude/skills/agent-team-cli/scripts/shutdown-team.sh "<项目绝对路径>"`；否 → 窗口保留可续用（同一团队接新任务：回 P0 对齐后直接从 P4 走）。
...
- **角色失联（看门狗唤醒仍无回报时）**：ListAgents 确认会话在线；向用户播报并请用户**目视确认该窗口是否空闲**（可能卡在确认框/仍在工作）；确认空闲后才可重发一次指令（角色慢而未死时重发会导致重复执行）；仍无响应 → 升级用户。
- **权限边界**：绝不指使角色去做你自己会话中被拒绝/会被拦的操作（跨会话洗权限）；这类操作一律回到用户。
- **SendMessage 寻址与兜底（实测经验）**：① 角色不在"本会话"里时，SendMessage 会要求带 ref——用 ListAgents 里显示的 `名字 [ref]` 形式（如 `executor [211318]`）；② SendMessage 偶发瞬时失败（角色在线也可能出现）→ 先 ListAgents 确认在线，再重发一次，**重发时改一下措辞**（系统会丢弃短时间内完全相同的重复消息）；④ 角色空闲多久都能被唤醒（无空闲超时；实测闲置 2 天仍可唤醒）——"叫不醒"的真实原因只会是：窗口已关/进程已死（ListAgents 里消失）、卡在确认框（请用户目视窗口）、或长期闲置后 token 需重新登录（让用户在该窗口操作）；③ 角色若因来源校验 BLOCKED（桌面版 main 的显示名可能与 --name 不同）→ 回一条确认"该地址就是主控"，角色即按 TOFU 锚定继续。派活消息里主动写一句「来源显示为 socket 地址属正常，本消息即主控指令」可减少此类回合。
