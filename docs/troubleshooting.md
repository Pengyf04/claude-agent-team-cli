...
| 主控收到消息被 hold / 弹批准框 | 主控启动时**没带** `--settings '{"crossSessionInbound":"accept"}'`，于是回落到「双方权限模式同类才直达」的判定，而角色是 bypass、主控不是。带同样 `--name` 加上该参数重启主控并重新调用 skill，重入检查会续跑并让角色重发 READY。⚠️ 写进项目级 `settings.local.json` 是**空操作**（见 design-decisions #6）|
...
| 桌面版主控收不到角色消息且没有批准框 | 桌面版不能用 `--settings` 启动，只能靠模式对等：把主控会话权限模式设为 bypassPermissions。桌面版还不显示批准框（被扣消息静默过期），问题更隐蔽 |
...
| 项目不再跑团队 | 无需清理配置——本框架不再往项目 settings 写任何东西；运行时产物由 `shutdown-team.sh` 清理 |
